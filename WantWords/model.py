...
class Encoder(torch.nn.Module):
    ...
    def forward(self, operation, x=None, w=None, ws=None, wl=None, wr=None, msk_s=None, msk_l=None, msk_r=None,
                mode=None):
        ...
        ''' word prediction '''
        # vd: T(bat, embed_dim)
        h_1 = torch.sum(h * alpha, 1)  # + torch.sum(self.embedding(x), 1)  # + torch.sum(x_embedding, 1)  # ok
        # h_1 = torch.sum(torch.cat([h, self.embedding(x)], 2) * alpha, 1)  # best
        vd = self.fc(h_1)
